...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        const [settings] = await pool.query("SELECT setting_value FROM system_settings WHERE setting_key = 'current_phase'");
        if (settings[0]?.setting_value !== '1') {
            res.status(403).json({ error: 'Registration is currently closed.' });
            return;
        }
        // 2. Insert into MySQL
        const [result] = await pool.query(`INSERT INTO contestants 
        ...
        await pool.query("UPDATE contestants SET contestant_number = ?, referral_code = ? WHERE id = ?", [contestantNumber, referralCode, contestantId]);
        res.status(201).json({
...
router.patch('/:id/status', protectAdmin, async (req, res) => {
    try {
        ...
        await pool.query('UPDATE contestants SET status = ?, rejection_reason = ?, reviewed_at = CURRENT_TIMESTAMP WHERE id = ?', [status, rejection_reason || null, id]);
