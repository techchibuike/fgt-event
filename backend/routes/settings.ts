 ...
 * @access  Public
 */
router.get('/phase', async (req: Request, res: Response) => {
    try {
        const [rows]: any = await pool.query(
            'SELECT setting_key, setting_value FROM system_settings WHERE setting_key IN (?, ?)',
            ['current_phase', 'voting_active']
        );
    ...
    try {
        if (phase) {
            await pool.query(
                ...
                ['current_phase', phase.toString(), phase.toString()]
            );
        }
        ...
        if (voting_active !== undefined) {
            await pool.query(
                'INSERT INTO system_settings (setting_key, setting_value) VALUES (?, ?) ON DUPLICATE KEY UPDATE setting_value = ?',
                ['voting_active', voting_active.toString(), voting_active.toString()]
            );
        }
