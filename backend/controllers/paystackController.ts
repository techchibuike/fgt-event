 ...
 * @access  Public
 */
export const initializeVotePayment = async (req: Request, res: Response) => {
    try {
        ...
        const [settings]: any = await pool.query(
            "SELECT setting_value FROM system_settings WHERE setting_key = 'voting_active'"
        );
    ...
    try {
        ...
        if (event.event === 'charge.success') {
            ...
            if (metadata.type === 'VOTE') {
                ...
                // Get current phase
                const [phaseRow]: any = await pool.query("SELECT setting_value FROM system_settings WHERE setting_key = 'current_phase'");
                ...
                const [result]: any = await pool.query(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)`,
                    [metadata.contestant_id, metadata.contestant_name, metadata.vote_quantity, currentPhase, customer.email, amountPaidNGN, reference]
                );
                ...
                if (result.affectedRows > 0) {
                    ...
                    await pool.query(
                        ...
                        [metadata.vote_quantity, metadata.contestant_id]
                    );
